...
const ManageProduct = () => {
    ...
    useEffect(() => {
        fetch(`http://s.rgvturf.com/products`)
        .then(res => res.json())
        .then(data => {
            setProducts(data)
    ...
    const deleteProduct = (id) => {
        ...
        if(proceed){
            const url = `http://s.rgvturf.com/products/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                    ...
                    products?.map(product => {
                        return  <div key={product._id} className='border my-2 d-md-flex justify-content-between align-items-center p-2 rounded'>
                                    <div style={{height: '50px'}} className='d-flex justify-content-between'>
                                        <img style={{height: '-webkit-fill-available', width: '60px', borderRadius: '5px', marginRight: '10px'}} src={`http://s.rgvturf.com/${product?.img}`} alt="" />
                                        {

...
const ManageProduct = () => {
    ...
    useEffect(() => {
        fetch(`https://s.rgvturf.com/products`)
        .then(res => res.json())
        .then(data => {
            setProducts(data)
    ...
    const deleteProduct = (id) => {
        ...
        if(proceed){
            const url = `https://s.rgvturf.com/products/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                    ...
                    products?.map(product => {
                        return  <div key={product._id} className='border my-2 d-md-flex justify-content-between align-items-center p-2 rounded'>
                                    <div style={{height: '50px'}} className='d-flex justify-content-between'>
                                        <img style={{height: '-webkit-fill-available', width: '60px', borderRadius: '5px', marginRight: '10px'}} src={`https://s.rgvturf.com/${product?.img}`} alt="" />
                                        {
                                    ...
                                    <div className='d-flex justify-content-end'>
                                        <a href={`https://rgfclient.web.app/product-catalog/${product.ProductTitle.replaceAll(' ','-')}/${product._id}`} target='_blank' rel='noreferrer'><FaExternalLinkAlt className='mx-2 icon_style_manage_page' data-bs-toggle="tooltip" data-bs-placement="top" title="Preview Product"/></a>
